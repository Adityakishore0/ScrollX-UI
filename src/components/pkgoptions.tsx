...
import { Copy, Check, AlertCircle, Terminal, ChevronDown } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function PkgOptions({ name }: PkgOptionsProps) {
  ...
  const copyToClipboard = async (): Promise<void> => {
    ...
    if (!success) success = legacyCopy(installCommand);
        ...
        <div className='flex-1 min-w-0 overflow-x-auto scrollbar-hide'>
          <div className='flex space-x-3 sm:space-x-6'>
            {packageManagers.map((pkg) => (
              <button
                key={pkg}
                className={`border-b-2 whitespace-nowrap transition-colors shrink-0 ${
                  selected === pkg
