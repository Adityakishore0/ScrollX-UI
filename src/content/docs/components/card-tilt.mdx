---
title: Card Tilt
description: smooth card tilt that responds naturally to your cursor, giving the card a more polished, modern look.
---

<ComponentPreview
  name="cardtilt-demo"
  className="[&_.preview>[data-orientation=vertical]]:sm:max-w-[70%]"
  description="An interactive card component with 3d tilt hover effect"
/>

## Installation

<Tabs defaultValue="cli" className="w-full">
  <TabsList>
    <TabsTrigger value="cli">CLI</TabsTrigger>
    <TabsTrigger value="manual">Manual</TabsTrigger>
  </TabsList>
  <TabsContent value="cli">
    <PkgOptions name="card-tilt" />
  </TabsContent>
  <TabsContent value="manual">
    <Steps>

<Step>Install the following dependencies:</Step>
<DepsOptions name="framer-motion" />

<Step>Copy and paste the following code into your project.</Step>
<p>
  <code>card-tilt.tsx</code>
</p>
<ComponentSource name="card-tilt" />

<Step>Add util file</Step>

<p>
  <code>lib/utils.ts</code>
</p>

```tsx
import { ClassValue, clsx } from "clsx";
import { twMerge } from "tailwind-merge";

export function cn(...inputs: ClassValue[]) {
  return twMerge(clsx(inputs));
}
```

</Steps>
  </TabsContent>
</Tabs>

## Usage

```tsx
import { CardTilt, CardTiltContent } from "@/components/ui/card-tilt";
```

```tsx
<CardTilt className="w-80" tiltMaxAngle={15} scale={1.05}>
      <CardTiltContent className="relative rounded-2xl bg-white p-6 shadow-xl">
        <h2 className="text-lg font-semibold mb-2">Interactive Tilt</h2>
        <p className="text-sm text-slate-600">
          Move your cursor over this card to see the 3D tilt effect.
        </p>
      </CardTiltContent>
    </CardTilt>
```

## API Reference

### Card Tilt

smooth card tilt that responds naturally to your cursor, giving the card a more polished, modern look.

## Props

<PropsTable
  rows={[
    {
      prop: "children",
      type: "ReactNode",
      default: "—",
      description: "Content rendered inside the CardTilt container."
    },
    {
      prop: "className",
      type: "string",
      default: "—",
      description: "Optional class for custom styling."
    },
    {
      prop: "tiltMaxAngle",
      type: "number",
      default: "15",
      description: "Maximum rotation angle applied when tilting."
    },
    {
      prop: "tiltReverse",
      type: "boolean",
      default: "false",
      description: "Reverses the tilt direction when true."
    },
    {
      prop: "glareEnable",
      type: "boolean",
      default: "false",
      description: "Reserved for future glare effect support."
    },
    {
      prop: "scale",
      type: "number",
      default: "1.05",
      description: "Scale value applied on hover tilt interaction."
    },
    {
      prop: "ref",
      type: "React.Ref<HTMLDivElement>",
      default: "—",
      description: "Returns the container element reference."
    }
  ]}
/>


