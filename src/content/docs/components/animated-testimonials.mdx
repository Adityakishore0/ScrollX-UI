...
Props that can be passed to the `AnimatedTestimonials` component.

- `data` – **Required.** An array of testimonial objects to display. Each object must include the following properties:
  - `name` *(string)* – Author's full name.
  - `image` *(string)* – URL of the author's profile picture.
  - `description` *(string)* – The testimonial content text.
  - `handle` *(string)* – Author's social media handle.
